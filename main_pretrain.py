...
if __name__ == "__main__":
    ...
    for step in range(n_epoch):
        ...
        # train
        loss = train_epoch_BERT(config, epoch, model, criterion_lm, criterion_cls, optimizer,
                                train_loader, n_epoch, scheduler)
        losses.append(loss)

        score_cls, score_lm = eval_epoch_bert(config, model, epoch, dev_loader)
        scores_cls.append(score_cls)
        scores_lm.append(score_lm)

        if score_cls > best_acc_cls and score_lm > best_acc_lm:
            print(f"pretrain model_{epoch} save...")
